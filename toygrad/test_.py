...
def test_cross_entropy():
    ...
    def single_cross_entropy(num_batches, nn_dim_out):
        ...
        y_prob = np.exp(y_logits)/np.sum(np.exp(y_logits), axis=-1, keepdims=True)
        ...
        outtorch = torch.nn.functional.cross_entropy(xtorch_logits, ytorch_prob)
...
def test_matmul():
    ...
    test_single_matmul((s3[2], ), (s3[6], s3[5], s3[4], s3[2], s3[3])) # Broadcasting
    # Matrix-Vector product
    s4 = np.random.randint(1, 20, size=(7,))
    test_single_matmul((s4[0], s4[1]), (s4[1], )) # No broadcasting
    test_single_matmul((s4[6], s4[4], s4[5], s4[2], s4[3]), (s4[3], )) # Broadcasting
